...
public class JournalJdbc {
    ...
    public List<Journal> getByStudent(int student_id) {
        List<Journal> journalList = jdbcTemplate.query(
                "select j.id, student_id, study_plan_id, in_time, count, mark_id " +
                "from journal j inner join student s on j.student_id=s.id where student_id = ?",
                this::mapJournal, student_id);
        return journalList;
    }
